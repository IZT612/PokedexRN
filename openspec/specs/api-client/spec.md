...
### Requirement: Shared PokeAPI client
The system SHALL provide a shared API client module for PokeAPI requests that is reusable by feature data modules.

#### Scenario: Client is configured for PokeAPI
...
The shared API client MUST enforce a fixed `10000` millisecond timeout on outbound requests so stalled calls fail predictably instead of hanging indefinitely.
...
The shared API client MUST convert transport and unexpected response failures into a predictable application-facing plain-object typed error.
...
- **THEN** it SHALL receive a plain-object typed error and SHALL NOT need to depend on class-based error instances
...
- **THEN** they SHALL verify the returned error matches the plain-object typed error contract
