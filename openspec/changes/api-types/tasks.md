...
- [ ] 1.3 Add `Pokemon` in the shared data entities module with `id`, `name`, a minimal `sprites` object that includes `front_default`, `types`, `stats`, `abilities`, `weight`, and `height`.
...
- [ ] 2.4 Add `PokemonDetailResponse` in the pokemon-detail feature data layer as a separate interface rather than reusing `Pokemon` directly.
