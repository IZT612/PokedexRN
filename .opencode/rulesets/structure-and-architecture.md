...
- Build UI with Tamagui when the needed primitive or layout can be expressed with the existing Tamagui setup; do not introduce parallel React Native view styling for normal UI work unless Tamagui cannot support the requirement cleanly.
- Reuse existing theme tokens from `src/theme/tokens.ts` instead of introducing ad hoc colors, spacing, or typography values.
- Do not hardcode new UI colors in components or feature files. Add shared colors to `src/theme/tokens.ts` first, then consume them through theme or UI token mappings.
